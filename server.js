...
const doctorToken = (req, res, next) => {
  const token = req.headers && req.headers["authorization"].split(" ").pop();
  const decode = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET);
...
courseRoute.get("/allCourses", authenticToken, (req, res) => {
  res.json(courses);
});

courseRoute.get("/myCourses", doctorToken, (req, res) => {
  const doctorCourse = [];
...
courseRoute.post("/Course", authenticToken, (req, res) => {
  courses.push({
    name: req.body.name,
    id: req.body.id,
